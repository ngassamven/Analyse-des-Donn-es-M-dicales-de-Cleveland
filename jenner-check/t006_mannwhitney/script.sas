...
/************************************* ANALYSES BIVARIEES *************************************/
/* Test de Mann-Whitney (Wilcoxon) — applique si Shapiro-Wilk rejette la normalite (macro de l'auteur)
   H0 : pas de difference significative (p > 0,05) ; H1 : difference significative (p < 0,05) */
%MACRO MannWhitney(variable_interet, variable_explicative);
PROC NPAR1WAY DATA = donnees WILCOXON;
...
%MannWhitney(Maladie, Age);
%MannWhitney(Maladie, Tension);
%MannWhitney(Maladie, Cholesterol);
